...
function pricePaths(a,stale){
 ...
 <article><h3 class="uptext">Bull case</h3><strong>Reclaim ${level(p.bull.trigger)}</strong><p>${escape(p.bull.rule)}</p><small>TP1 ${level(p.bull.objective)} · TP2 ${level(p.bull.second_objective)}<br>Failed-break invalidation: ${level(p.bull.invalidation)}</small></article>
 ...
 <article><h3 class="downtext">Bear case</h3><strong>Lose ${level(p.bear.trigger)}</strong><p>${escape(p.bear.rule)}</p><small>TP1 ${level(p.bear.objective)} · TP2 ${level(p.bear.second_objective)}<br>Failed-break invalidation: ${level(p.bear.invalidation)}</small></article></div><p class="footnote">${escape(p.note)}</p></section>`;
}
...
function technicalBrief(a,stale){
 const b=a.technical_brief;if(!b)return '';
 return `<section class="panel span12" id="technical-brief"><div class="panel-head"><h2>Current technicals · what matters now</h2>${badge(stale?'STALE · WAIT':b.alignment+' ALIGNMENT')}</div><div class="path-grid">${b.rows.map(r=>`<article data-technical-frame="${r.timeframe}"><h3>${r.timeframe} · ${escape(r.bias)}</h3><small>Completed ${time(r.known_at)} · ${r.fresh?'Latest bar available':'LATEST BAR MISSING'}</small><p>${escape(r.momentum)} momentum · RSI ${n(r.rsi,1)}<br>${escape(r.trend_strength)} · ADX ${n(r.adx,1)}</p><p>Support <strong>${Number.isFinite(r.support)?'$'+n(r.support):'Unconfirmed'}</strong><br>Resistance <strong>${Number.isFinite(r.resistance)?'$'+n(r.resistance):'Unconfirmed'}</strong></p><small>ATR $${n(r.atr)} · EMA20 $${n(r.ema20)}<br>Distance from EMA20: ${n(r.extension_atr,1)} ATR${r.extension_warning?' · EXTENDED; avoid chasing':''}</small></article>`).join('')}</div><p class="footnote">${escape(b.note)}</p></section>`;
}
