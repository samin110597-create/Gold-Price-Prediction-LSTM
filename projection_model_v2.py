...
def download(ticker,period,interval):
    ...
    attempts=[]
    if interval=='1h':
        attempts=[{'period':'2y'},{'period':'1y'}]
    else:
        # Use the longest available history for stable daily / annual validation.
        attempts=[{'start':'2000-01-01'},{'period':'max'},{'period':period}]
    for kw in attempts:
        try:
            df=flatten(yf.download(ticker,interval=interval,auto_adjust=False,progress=False,threads=False,**kw))
...
def origins(n,steps,intraday,label):
    if intraday:
        min_train,spacing,maxn=1600,24,96
    elif label=='1 Year':
        # Annual labels are heavily overlapping if sampled every few weeks. Use roughly
        # non-overlapping one-year origins and long history instead.
        min_train,spacing,maxn=1260,252,22
    else:
        # A larger multi-year evaluation window makes 1D/1W quality less sensitive
        # to a single recent regime while remaining chronological and purged.
        min_train,spacing,maxn=900,10,96
...
def project(df,steps,label,intraday=False):
    ...
    # Stability diagnostics: the recent half must not be catastrophically opposite to
    # the full-sample result. These are reported rather than used to loosen PASS.
    half=max(12,len(records)//2);ra=actual[-half:];re=ens[-half:];rb=basepred[-half:];rmae=float(mean_absolute_error(ra,re));rbmae=float(mean_absolute_error(ra,rb));recent_skill=1-rmae/max(rbmae,1e-9);recent_edge=float(np.mean((re>=0)==(ra>=0))-np.mean((rb>=0)==(ra>=0)))
    ...
    conf='High' if edge>=.06 and skill>=.10 and agree>=.67 and len(records)>=48 else ('Moderate' if edge>=.03 and skill>=.04 and agree>=.67 and len(records)>=36 else 'Low')
    if label=='1 Year':conf='Moderate' if (edge>=.03 and skill>=.10 and len(records)>=15) else 'Low'
    passed=bool(edge>0 and skill>0 and len(records)>=12)
    return {'horizon':label,'steps':steps,'predicted_price':round(price,2),'model_price':round(price,2),'raw_ml_price':round(price,2),'projected_return_pct':round(pret*100,3),'probability_up':round(pup,4),'probability_down':round(1-pup,4),'confidence':conf,'confidence_score':70 if conf=='High' else (58 if conf=='Moderate' else 38),'forecast_status':'Validated' if passed else 'Estimate only','tight_model_zone':[round(float(low),2),round(float(high),2)],'focus_zone':[round(float(low),2),round(float(high),2)],'risk_zone':[round(float(low),2),round(float(high),2)],'zone_target_coverage':coverage,'backtest_directional_accuracy':round(dacc,4),'baseline_directional_accuracy':round(base_dir,4),'directional_edge':round(edge,4),'backtest_mae_pct':round(mae*100,3),'baseline_mae_pct':round(base_mae*100,3),'mae_skill_vs_baseline':round(skill,4),'recent_half_directional_edge':round(recent_edge,4),'recent_half_mae_skill':round(recent_skill,4),'model_agreement':round(agree,4),'walkforward_origins':len(records),'validation':'stabilized purged expanding-origin walk-forward; larger OOS sample; 1Y uses approximately non-overlapping annual origins','model_version':'stabilized champion V5','models':detail}

def run_asset(key,cfg):
    d=download(cfg['ticker'],'10y','1d');h=download(cfg['ticker'],'2y','1h');ps=[project(h,4,'4 Hours',True),project(d,1,'1 Day'),project(d,5,'1 Week'),project(d,252,'1 Year')];payload={'status':'ok','asset':key,'symbol':cfg['ticker'],'instrument':cfg['name'],'updated_utc':datetime.now(timezone.utc).isoformat(),'latest_price':round(float(h.Close.iloc[-1]),2),'projections':ps,'model_version':'stabilized champion V5','note':'Larger purged OOS samples reduce score volatility. 1Y uses long history and approximately non-overlapping annual origins. PASS thresholds are not loosened.'};(OUT_DIR/cfg['output']).write_text(json.dumps(payload,indent=2),encoding='utf-8')
