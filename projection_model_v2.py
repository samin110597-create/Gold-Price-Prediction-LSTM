...
def templates(intraday=False):
    ...
    return {
        ...
        "Huber": Pipeline([("scale", StandardScaler()), ("model", HuberRegressor(epsilon=1.35, alpha=0.001, max_iter=500))]),
        "Random Forest": RandomForestRegressor(n_estimators=220, max_depth=6, min_samples_leaf=min_leaf, max_features="sqrt", random_state=42, n_jobs=-1),
        "Extra Trees": ExtraTreesRegressor(n_estimators=220, max_depth=7, min_samples_leaf=min_leaf, max_features="sqrt", random_state=43, n_jobs=-1),
        "Gradient Boost": HistGradientBoostingRegressor(max_iter=160, max_depth=3, learning_rate=0.035, l2_regularization=0.6, random_state=42),
    }


def origins(n, steps, intraday, label):
    if intraday:
        min_train, spacing, maxn = 1500, 24, 36
    elif label == "1 Year":
        min_train, spacing, maxn = 1260, 42, 18
    else:
        min_train, spacing, maxn = 900, 5, 48
...
def run_asset(key, cfg):
    ...
    payload = {
        ...
        "latest_price": round(float(hourly["Close"].iloc[-1]), 2),
        "projections": projections,
        "note": "Calibrated price model. The point forecast is shrunk toward a simple baseline when walk-forward evidence is weak. Focus zones are narrower OOS-error bands; risk zones preserve wider uncertainty. Research/education only.",
    }
