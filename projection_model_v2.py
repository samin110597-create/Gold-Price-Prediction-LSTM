...
def download(ticker,period,interval):
    df=flatten(yf.download(ticker,period=period,interval=interval,auto_adjust=False,progress=False,threads=False))
    if df.empty or 'Close' not in df.columns:raise RuntimeError(f'No data returned for {ticker} {interval}')
...
def project(df,steps,label,intraday=False):
    ...
    if label=='1 Year' and conf=='High':conf='Moderate'
    return {'horizon':label,'steps':steps,'model_price':round(price,2),'projected_return_pct':round(pret*100,3),'probability_up':round(pup,4),'probability_down':round(1-pup,4),'confidence':conf,'tight_model_zone':[round(float(low),2),round(float(high),2)],'zone_target_coverage':coverage,'backtest_directional_accuracy':round(dacc,4),'baseline_directional_accuracy':round(base_dir,4),'directional_edge':round(edge,4),'backtest_mae_pct':round(mae*100,3),'baseline_mae_pct':round(base_mae*100,3),'mae_skill_vs_baseline':round(skill,4),'model_agreement':round(agree,4),'walkforward_origins':len(records),'validation':'purged expanding-origin walk-forward','models':detail}

def run_asset(key,cfg):
    d=download(cfg['ticker'],'10y','1d');h=download(cfg['ticker'],'730d','1h');ps=[project(h,4,'4 Hours',True),project(d,1,'1 Day'),project(d,5,'1 Week'),project(d,252,'1 Year')];payload={'status':'ok','asset':key,'symbol':cfg['ticker'],'instrument':cfg['name'],'updated_utc':datetime.now(timezone.utc).isoformat(),'latest_price':round(float(h.Close.iloc[-1]),2),'projections':ps,'note':'Purged walk-forward model. Zones are centered on model price and calibrated from out-of-sample residuals; 1-year confidence is capped because long-horizon samples are limited and regime-dependent.'};(OUT_DIR/cfg['output']).write_text(json.dumps(payload,indent=2),encoding='utf-8')
