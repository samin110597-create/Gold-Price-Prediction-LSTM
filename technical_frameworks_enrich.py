...
def pivots(df,order=3):
    ...
    for i in range(order,len(df)-order):
        if h[i]>=np.nanmax(h[i-order:i+order+1]):out.append({'i':i,'type':'H','price':float(h[i]),'date':str(pd.Timestamp(df.index[i]).date())})
        if l[i]<=np.nanmin(l[i-order:i+order+1]):out.append({'i':i,'type':'L','price':float(l[i]),'date':str(pd.Timestamp(df.index[i]).date())})
...
def zones(df,pv):
    ...
    def side(kind,where):
        ...
        rows=[{'center':round(c['center'],2),'low':round(c['center']-tol*.65,2),'high':round(c['center']+tol*.65,2),'touches':len(c['vals']),'strength':'Strong' if len(c['vals'])>=3 else ('Medium' if len(c['vals'])==2 else 'Light')} for c in cl]
        rows.sort(key=lambda x:(-x['touches'],abs(x['center']-last)));return rows[:3]
    return {'supports':side('L','support'),'resistances':side('H','resistance')}

def fibonacci(df):
    w=df.tail(120);hi=float(w.High.max());lo=float(w.Low.min());hi_i=int(w.High.values.argmax());lo_i=int(w.Low.values.argmin());up=lo_i<hi_i;r=hi-lo
...
def zigzag(df):
    ...
    for i in range(start+1,len(df)):
        ...
        if direction>=0:
            ...
            if p/ext-1<=-threshold:zz.append({'type':'H','price':round(ext,2),'date':str(pd.Timestamp(df.index[ext_i]).date())});direction=-1;ext=p;ext_i=i
        if direction<=0:
            if p<ext:ext=p;ext_i=i
            if p/ext-1>=threshold:zz.append({'type':'L','price':round(ext,2),'date':str(pd.Timestamp(df.index[ext_i]).date())});direction=1;ext=p;ext_i=i
    return zz[-8:]
...
def elliott(zz):
    ...
    return {'phase':phase,'bias':bias,'confidence':conf,'points':[dict(x,label=labels[i]) for i,x in enumerate(recent)],'note':'Interpretive swing framework; alternate wave counts are possible.'}
...
def wyckoff(df,volume_flow):
    ...
    return {'phase':phase,'bias':bias,'confidence':conf,'range_position':round(pos,3),'range_low':round(lo,2),'range_high':round(hi,2),'evidence':evidence,'note':'Rule-based Wyckoff-style context, not a definitive phase label.'}
...
def enrich(asset,ticker,file):
    ...
    payload['support_resistance']=zones(df,pv);payload['fibonacci_deep']=fibonacci(df);payload['elliott_wave']=elliott(zz);payload['wyckoff']=wyckoff(df,payload.get('volume_flow'))
    old={x.get('date'):x for x in payload.get('chart',[]) if x.get('date')}
    chart=[]
    for idx,row in df.tail(180).iterrows():
        d=str(pd.Timestamp(idx).date());base=old.get(d,{})
        base.update({'date':d,'open':round(float(row.Open),2),'high':round(float(row.High),2),'low':round(float(row.Low),2),'close':round(float(row.Close),2)})
        chart.append(base)
    payload['chart']=chart
    payload['frameworks_updated_utc']=datetime.now(timezone.utc).isoformat();payload['framework_note']='Support/resistance are zones. Elliott and Wyckoff are interpretive frameworks and are not guaranteed predictions.'
